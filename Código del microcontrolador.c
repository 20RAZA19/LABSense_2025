...
const char* ssid = "ADMINISTRACION";
const char* password = "032021@dmin1n";
...
const float SMOKE_A = 305.33;
const float SMOKE_B = -3.401;
const float H2_A = 98.866;
const float H2_B = -2.732;
const float ALCOHOL_A = 0.4;
const float ALCOHOL_B = -1.5;
const float BENZENE_A = 0.2;
const float BENZENE_B = -1.4;
const float CO_A = 99.042;
const float CO_B = -1.518;
const float TOLUENE_A = 4.83;
const float TOLUENE_B = -2.62;
const float AMMONIA_A = 102.2;
const float AMMONIA_B = -2.473;
const float CO2_A = 116.602;
...
String HandleResponse(String query) {
  ...
  if (query == "/data dht2") {
    ...
    if (isnan(h) || isnan(t)) {
      return "Error: No se pudo leer la información del sensor DHT22.";
    }
    return formatResponse("DHT22", "🌡️ Temperatura: " + String(t, 2) + " °C\n💧 Humedad: " + String(h, 2) + " %");
...
void loop() {
  ...
  // Página 1 (Dashboard)
  sprintf(buffer, "page1.temp.txt=\"%.1f\"", temperatura);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.hum.txt=\"%.1f\"", humedad);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.smoke.txt=\"%d\"", humo_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.lpg.txt=\"%d\"", lpg_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.h2.txt=\"%d\"", h2_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.Alcohol.txt=\"%.2f\"", alcohol_mgL);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.Benceno.txt=\"%.2f\"", benceno_mgL);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.CO.txt=\"%d\"", co_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.CO2.txt=\"%d\"", co2_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.Tolueno.txt=\"%d\"", tolueno_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.Amoniaco.txt=\"%d\"", amoniaco_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page1.ICA.txt=\"%d\"", ica_valor);
  sendCommandToNextion(buffer);

  long temp_progress = map(temperatura, 0, 50, 0, 100);
  sprintf(buffer, "page1.j1.val=%ld", temp_progress);
  sendCommandToNextion(buffer);

  long lpg_angle = map(lpg_ppm, 0, 1000, 0, 180);
  sprintf(buffer, "glpg.val=%ld", lpg_angle);
  sendCommandToNextion(buffer);

  long alcohol_angle_dash = map(alcohol_mgL * 1000, 0, 1500, 0, 180);
  sprintf(buffer, "galc.val=%ld", alcohol_angle_dash);
  sendCommandToNextion(buffer);

  long ica_angle_dash = map(ica_valor, 0, 500, 0, 180);
  sprintf(buffer, "gica.val=%ld", ica_angle_dash);
  sendCommandToNextion(buffer);
  ...
  sprintf(buffer, "page2.temp.txt=\"%.1f\"", temperatura);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page2.hum.txt=\"%.1f\"", humedad);
  sendCommandToNextion(buffer);
  long temp_progress_p2 = map(temperatura, 0, 50, 0, 100);
  long hum_progress_p2 = map(humedad, 20, 80, 0, 100);
  sprintf(buffer, "page2.j0.val=%ld", temp_progress_p2);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page2.j1.val=%ld", hum_progress_p2);
  sendCommandToNextion(buffer);
  ...
  // Página 3 (MQ2)
  sprintf(buffer, "page3.Smoke.txt=\"%d\"", humo_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page3.LPG.txt=\"%d\"", lpg_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page3.H2.txt=\"%d\"", h2_ppm);
  sendCommandToNextion(buffer);

  long smoke_angle_p3 = map(humo_ppm, 0, 1000, 0, 180);
  sprintf(buffer, "g_smoke_p3.val=%ld", smoke_angle_p3);
  sendCommandToNextion(buffer);
  ...
  if (!p3_s0_paused) {
    ...
    sprintf(buffer, "add page3.s0.id,0,%d", smoke_waveform);
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
    sprintf(buffer, "add page3.s0.id,1,%d", lpg_waveform);
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
    sprintf(buffer, "add page3.s0.id,2,%d", h2_waveform);
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
  }

  // Página 4 (MQ-3)
  sprintf(buffer, "page4.Alcohol.txt=\"%.2f\"", alcohol_mgL);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page4.Benceno.txt=\"%.2f\"", benceno_mgL);
  sendCommandToNextion(buffer);

  long alcohol_angle_p4 = map(alcohol_mgL * 1000, 0, 1500, 0, 180);
  sprintf(buffer, "g_alc_p4.val=%ld", alcohol_angle_p4);
  sendCommandToNextion(buffer);

  if (!p4_s0_paused) {
    int alcohol_waveform = map(alcohol_mgL, 0, 10, 0, 255);
    int benceno_waveform = map(benceno_mgL, 0, 10, 0, 255);
    sprintf(buffer, "add page4.s0.id,0,%d", constrain(alcohol_waveform, 0, 255));
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
    sprintf(buffer, "add page4.s0.id,1,%d", constrain(benceno_waveform, 0, 255));
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
  }

  // Página 5 (MQ-3)
  sprintf(buffer, "page5.Alcohol.txt=\"%.2f\"", alcohol_mgL);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page5.Benceno.txt=\"%.2f\"", benceno_mgL);
  sendCommandToNextion(buffer);

  long alcohol_angle_p5 = map(alcohol_mgL * 1000, 0, 1500, 0, 180);
  sprintf(buffer, "g_alc_p5.val=%ld", alcohol_angle_p5);
  sendCommandToNextion(buffer);

  if (!p5_s0_paused) {
    int alcohol_waveform = map(alcohol_mgL, 0, 4, 0, 255);
    int benceno_waveform = map(benceno_mgL, 0, 4, 0, 255);
    sprintf(buffer, "add page5.s0.id,0,%d", constrain(alcohol_waveform, 0, 255));
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
    sprintf(buffer, "add page5.s0.id,1,%d", constrain(benceno_waveform, 0, 255));
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
  }

  // Página 7 (MQ-7)
  sprintf(buffer, "page7.CO.txt=\"%d\"", co_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page7.CO2.txt=\"%d\"", co2_ppm);
  sendCommandToNextion(buffer);
  if (!p7_s0_paused) {
    int co_waveform = map(co_ppm, 0, 1000, 0, 255);
    int co2_waveform = map(co2_ppm, 0, 1000, 0, 255);
    sprintf(buffer, "add page7.s0.id,0,%d", constrain(co_waveform, 0, 255));
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
    sprintf(buffer, "add page7.s0.id,1,%d", constrain(co2_waveform, 0, 255));
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
  }

  // Página 8 (MQ-135 Gráficas)
  sprintf(buffer, "page8.Tolueno.txt=\"%d\"", tolueno_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page8.Amoniaco.txt=\"%d\"", amoniaco_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page8.ICA.txt=\"%d\"", ica_valor);
  sendCommandToNextion(buffer);
  if (!p8_s0_paused) {
    int tolueno_waveform = mapLog(tolueno_ppm, 100, 10000, 0, 255);
    int amoniaco_waveform = mapLog(amoniaco_ppm, 100, 10000, 0, 255);
    sprintf(buffer, "add page8.s0.id,0,%d", tolueno_waveform);
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
    sprintf(buffer, "add page8.s0.id,1,%d", amoniaco_waveform);
    for (int i = 0; i < 10; i++) { sendCommandToNextion(buffer); }
  }
  if (!p8_s1_paused) {
  ...
  sprintf(buffer, "page9.Tolueno.txt=\"%d\"", tolueno_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page9.Amoniaco.txt=\"%d\"", amoniaco_ppm);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page9.ICA.txt=\"%d\"", ica_valor);
  sendCommandToNextion(buffer);
  long tolueno_progress = constrain((long)tolueno_ppm / 100, 0, 100);
  long amoniaco_progress = constrain((long)amoniaco_ppm / 100, 0, 100);
  sprintf(buffer, "page9.j0.val=%ld", tolueno_progress);
  sendCommandToNextion(buffer);
  sprintf(buffer, "page9.j1.val=%ld", amoniaco_progress);
  sendCommandToNextion(buffer);

  long ica_angle = map(ica_valor, 0, 500, 0, 180);
  sprintf(buffer, "g_ica.val=%ld", ica_angle);
  sendCommandToNextion(buffer);
  ...
  Serial.println("PPM");
  Serial.print("Humo: ");
  Serial.print(humo_ppm);
  Serial.println("PPM");
  Serial.print("Alcohol: ");
  Serial.print(alcohol_mgL);
  Serial.println("PPM");
  Serial.print("Benceno: ");
  Serial.print(benceno_mgL);
  Serial.println("PPM");
  Serial.print("CO: ");
  Serial.print(co_ppm);
  Serial.println("PPM");
  ...
  Serial.println("PPM");
  Serial.print("Amoníaco: ");
  Serial.print(amoniaco_ppm);
  Serial.println("PPM");
  Serial.print("Tolueno: ");
  Serial.print(tolueno_ppm);
  Serial.println("PPM");
  Serial.print("ICA: ");
  Serial.print(ica_valor);
  sendToGoogleSheet();
  delay(250);
}
